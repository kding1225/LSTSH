...
%-------------------------------------------------------------------------
%testing
if(size(tr_dat,1)*size(W,2)*8/1024/1024/1024>1.5)
    labels = zeros(size(tr_dat,1),1);
    bs = 100;
    nb = ceil(length(labels)/bs);
    E = 0;
    for i = 1:nb
        ind = (i-1)*bs+1:min(i*bs,length(labels));
        if nargout > 1
            [~,labels(ind)] = max(tr_dat(ind,:)*W, [], 2);
        end
        if nargout > 2
            E = E + sum(sum((Y(ind,:) - tr_dat(ind,:)*W).^2));
        end
    end
    if nargout>2
        E = E + lambda*sum(sum(W.^2));
    end
else
    if nargout > 1
